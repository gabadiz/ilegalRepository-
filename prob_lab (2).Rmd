...
author: ¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡NO TE OLVIDES!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Nombre1 Apellido1
  (email1@usp.ceu.es), Nombre2 Apellido2 (email2@usp.ceu.es), etc
output:
...
dist_prob = distribution_empty(n_servers, n_datablocks, n_sims)
print(dist_prob)

# Graficamos la función de probabilidad de E
plot(0:n_servers, 0:distribution_empty(n_servers, n_datablocks, n_sims), type = "h",
...
# Función que simula una única instancia de servidores vacíos
simulate_empty <- function(n_servers, n_datablocks) {
  paquetes <- sample(1:n_servers, n_datablocks, replace = TRUE)
  conteo <- table(factor(paquetes, levels = 1:n_servers))
  servidores_vacios <- sum(conteo == 0)
  return(servidores_vacios)
}

# Función que calcula la distribución de probabilidad completa para E
distribution_empty <- function(n_servers, n_datablocks, n_sims) {
  resultados <- replicate(n_sims, simulate_empty(n_servers, n_datablocks))
  tabla_resultados <- table(resultados) / n_sims  # Proporción de cada valor de E
...
# Calculamos la distribución de probabilidad para E
dist_prob <- distribution_empty(n_servers, n_datablocks, n_sims)
print(dist_prob)

# Graficamos la función de probabilidad de E
plot(as.numeric(names(dist_prob)), dist_prob, type = "h", 
     xlab = "Número de servidores vacíos (E)", 
     ylab = "Probabilidad", 
     main = "Distribución de la probabilidad de E",
     col = "blue", lwd = 2)
